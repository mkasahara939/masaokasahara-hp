...
sub User_Agent {
	...
	if($user_agent =~ /DoCoMo/i) {
		...
		if($user_agent =~ /Mobile\/([a-zA-Z0-9\-\.]+)/) {
			...
			if($user_agent =~ /Mac OS X ([\d\_]+)/) {
				$platform_v = $1;
				$platform_v =~ s/\_/\./g;
				($platform_v) = $platform_v =~ /^(\d+\.\d+)/;
			}
			...
			if($user_agent =~ /Windows\s+([^\;]+)/i) {
				$platform = "Windows";
				$platform_v = $1;
				if($platform_v eq 'NT 6.1') {
					$platform_v = '7';
				} elsif($platform_v eq 'NT 6.0') {
					$platform_v = 'Vista';
		...
		my %devid_list = (
			...
			'TS3U' => 'au,LIGHT POOL [iida]',
			'SN3O' => 'au,S003',
			'SN3P' => 'au,S004',
			'SH3H' => 'au,SH007',
			'KC3V' => 'au,lotta [iida]',
			'SH3F' => 'au,SH005',
			'KC3S' => 'au,PRISMOID [iida]',
			'CA3G' => 'au,CA004',
			'CA3F' => 'au,CA003',
			'HI3G' => 'au,Mobile Hi-Vision CAM Wooo',
			'TS3P' => 'au,T002',
			'SH3C' => 'au,Sportio water beat',
			'SN3J' => 'au,S001',
			'TS3N' => 'au,T001',
			'CA3D' => 'au,CA001',
			'KC3M' => 'au,K001',
	...
	} elsif($user_agent =~ /^Mozilla\/[^\(]+\(compatible\;\s+MSIE\s+.+\)/) {
		...
		} elsif($user_agent =~ /Windows NT 6\.1/i) {
			$platform = 'Windows';
			$platform_v = '7';
